<template>
  <div>
    <div class="search">
      <el-input clearable placeholder="请输入报修说明" style="width: 200px" v-model="name"></el-input>
    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" width="180" align="center">
          <template v-slot="scope">
            <el-button plain type="primary" @click="handleEdit(scope.row)" size="mini" v-if="user.role === 'LABADMIN' && scope.row.status === '待处理'">处理</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        username: [
          {required: false, message: '请输入检修人', trigger: 'blur'},
        ],
        phone: [
          {required: false, message: '请输入联系方式', trigger: 'blur'},
        ],
        content: [
          {required: false, message: '请输入检修说明', trigger: 'blur'},
        ],
        time: [
          {required: false, message: '请输入检修时间', trigger: 'blur'},
        ],
        department: [
          {required: false, message: '请输入检修单位', trigger: 'blur'},
        ],
      },
      ids: [],
      id: null,
